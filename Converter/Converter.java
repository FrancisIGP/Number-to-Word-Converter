    ...
    public static final void converter(String value) {
        ...
        for (int placeVal = 0, ctr = valueSize; placeVal <= outputSize - 1; placeVal++, tracker++, ctr--) {
            ...
            switch (tracker) {
                case 1:
                    if (placeVal == 0) {
                        output[placeVal] = WordList.wordFormat[0][currentVal]; // Ones 
                    } else {
                        output[placeVal] = WordList.wordFormat[0][currentVal].concat(" " + WordList.commas[commasCTR]); // Commas
                        commasCTR++;
                    }
                    break;
                case 2:
                    if (currentVal == 1) { // Teens
                        ...
                        output[placeVal] = WordList.wordFormat[2][currentVal];
                        output[placeVal - 1] = " ";
                    } else {
                        output[placeVal] = WordList.wordFormat[2][currentVal]; // Decades
                    }
                    break;
                case 3:
                    if (currentVal == 0) output[placeVal] = " "; 
                    else output[placeVal] = WordList.wordFormat[0][currentVal].concat(" " + WordList.commas[0]); // Hundredths
                    break;
            }
